...
class mainScraper():
    ...
    def totalScraping(self):
        ...
        input_xlsx = xlrd.open_workbook("Data/retotal_001.xlsx")
        ...
        if os.path.exists('Result/total.xlsx'):
            self.wb1 = openpyxl.load_workbook("Result/total.xlsx")
        else:
            self.wb1 = openpyxl.Workbook()
        self.ws1 = self.wb1.active

        self.wb2 = openpyxl.load_workbook("Data/retotal_001.xlsx")
    ...
    def scrapingByURL(self):
        ...
        if url not in self.temp_data and url is not "":
            self.temp_data.append(url)
        else:
            self.ws2.cell(row=index + 1, column=1).value = ""
            self.ws2.cell(row=index + 1, column=2).value = ""
            self.wb2.save("Data/retotal_001.xlsx")
            return
        ...
        logTxt = "\turl:\t{}".format(url)
        self.log_printer.print_log(logTxt)

        logTxt = "\tid:\t{}".format(id)
        self.log_printer.print_log(logTxt)

        logTxt = "\ttype:\t{}".format(type)
        self.log_printer.print_log(logTxt)

        logTxt = "\ttitle:\t{}".format(title)
        self.log_printer.print_log(logTxt)

        logTxt = "\tdesc:\t{}".format(desc)
        self.log_printer.print_log(logTxt)

        logTxt = "\tcolor:\t{}".format(color)
        self.log_printer.print_log(logTxt)

        logTxt = "\tall_price:\t{}".format(all_price)
        self.log_printer.print_log(logTxt)

        logTxt = "\tsale_price:\t{}".format(sale_price)
        self.log_printer.print_log(logTxt)

        self.total_index += 1
        self.ws1.cell(row=self.total_index, column=1).value = url
        self.ws1.cell(row=self.total_index, column=2).value = id
        self.ws1.cell(row=self.total_index, column=3).value = type
        self.ws1.cell(row=self.total_index, column=4).value = title
        self.ws1.cell(row=self.total_index, column=5).value = desc
        self.ws1.cell(row=self.total_index, column=6).value = color
        self.ws1.cell(row=self.total_index, column=7).value = all_price
        self.ws1.cell(row=self.total_index, column=8).value = sale_price
        self.wb1.save(self.dest + "/" + self.outfile_name)
